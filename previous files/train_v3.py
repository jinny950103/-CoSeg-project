"""
train_v3.py — 下顎神經管分割訓練腳本 (v3 — 目標 Dice 0.8+)
=============================================================
v2 → v3 改良重點：
  1. Dice 計算修正：空切片不計入平均，訓練時看到真實數字
  2. 更強的資料增強：大範圍仿射、更強彈性變形、Copy-Paste 增強
  3. 支援 7+ 病人訓練資料
  4. 按病人分 split（消除 data leakage）
  5. 動態正樣本權重 + Online Hard Example Mining
"""
...
OUTPUT_DIR = "outputs"
OUTPUT_WEIGHTS = os.path.join(OUTPUT_DIR, "coseg_v3_best.pth")
OUTPUT_LAST = os.path.join(OUTPUT_DIR, "coseg_v3_last.pth")
...
class FocalDiceBoundaryLoss(nn.Module):
    ...
    def boundary_loss(self, pred, target):
        ...
        # 在邊界區域計算 BCE（需關閉 autocast，因為 BCE 不支援 bfloat16）
...
def main():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if epoch % 3 != 0 and epoch != NUM_EPOCHS - 1:
            ...
            print(f"Epoch {epoch} | Train Loss: {train_loss:.4f} Dice: {train_dice:.4f} | Val: skipped | LR: {scheduler.get_lr()[0]:.2e}")
            continue
