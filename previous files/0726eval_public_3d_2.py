...
from scipy.ndimage import label  # 🌟 載入 3D 連通物件分析工具
...
def main():
    ...
    print("🚀 開始高速 3D 影像評估 (啟用 3D 後處理去雜訊，對比圖將輸出至 0726_compare 資料夾) ...")
    with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
        for patient_id in patients:
            ...
            # 🧹 3D 後處理：最大連通物件過濾機制
            # ==========================================
            pred_volume = np.stack(pred_slices)
            gt_volume = np.stack(gt_slices)
            
            labeled_pred, num_features = label(pred_volume)
            
            # 如果偵測到的碎塊數量大於 2，強制剔除多餘幻覺，只保留體積最大的兩個
            if num_features > 2:
                component_sizes = np.bincount(labeled_pred.ravel())
                component_sizes[0] = 0  # 忽略背景
                top_2_labels = component_sizes.argsort()[-2:]
                pred_volume = np.isin(labeled_pred, top_2_labels).astype(np.float32)
            # ==========================================

            dice, iou = compute_3d_metrics(pred_volume, gt_volume)
            if dice is not None:
