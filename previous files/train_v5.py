"""
train_v5.py — 下顎神經管分割訓練腳本 (v5 — 解凍更多層 + 分層 LR)
=============================================================
v4 → v5 改良重點：
  1. 載入 SAM2 預訓練權重
  2. 解凍 encoder 最後 2 個 stage（trunk.blocks 後半段）
  3. 分層 LR：encoder 用低 LR，預測頭用高 LR
  4. 保留 v3/v4 全部改進
"""
...
OUTPUT_DIR = "outputs"
OUTPUT_WEIGHTS = os.path.join(OUTPUT_DIR, "coseg_v5_best.pth")
OUTPUT_LAST = os.path.join(OUTPUT_DIR, "coseg_v5_last.pth")
...
def main():
    ...
    #   2. encoder 淺層 (trunk.blocks 後半 + edge + neck) → 低 LR
    #   3. 預測頭 (非 image_encoder) → 高 LR
    
    encoder_shallow_params = []  # 要訓練的 encoder 參數
    head_params = []             # 預測頭參數
    ...
    for n, p in model.named_parameters():
        if "image_encoder" in n:
            if "edge" in n or "neck" in n:
                ...
                # trunk.blocks: 解凍後半段
                # SAM2 Hiera-Large 有多個 blocks，解凍最後 1/3
                block_match = [s for s in n.split('.') if s.isdigit()]
                if block_match:
                    block_idx = int(block_match[0])
                    # Hiera-Large blocks 通常有 ~24 個，解凍最後 8 個
                    if block_idx >= 16:
                        p.requires_grad = True
                        encoder_shallow_params.append(p)
                    else:
    ...
    print(f"   Encoder 淺層: {sum(p.numel() for p in encoder_shallow_params)/1e6:.2f}M")
    print(f"   預測頭:       {sum(p.numel() for p in head_params)/1e6:.2f}M")

    # 分層 LR: encoder 用 1/10 的學習率
