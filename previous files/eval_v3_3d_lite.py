"""
eval_v3_3d.py — 下顎神經管 3D 評估（含管狀結構指標）
=====================================================
新增指標：clDice, Connectivity Rate, Breakpoints, Centerline Distance
"""

import os
import cv2
import torch
import numpy as np
import hydra
import torch.nn.functional as F
from tqdm import tqdm

from model import CoSeg
from sam2.build_sam import build_sam2
from tubular_metrics import compute_3d_dice, compute_3d_iou, compute_connectivity_rate, compute_num_components, compute_breakpoints

def compute_all_metrics(pred, gt):
    return {
        "dice": compute_3d_dice(pred, gt),
        "iou": compute_3d_iou(pred, gt),
        "connectivity_rate": compute_connectivity_rate(pred),
        "num_components": compute_num_components(pred),
        "breakpoints": compute_breakpoints(pred, gt),
        "pred_voxels": int((pred > 0).sum()),
        "gt_voxels": int((gt > 0).sum()),
    }

# ==========================================
# 📂 路徑設定
# ==========================================
PROJECT_ROOT = "/home/u9444861/-CoSeg-project"
EVAL_DATA_DIR = os.path.join(PROJECT_ROOT, "data/public_data/eval")
MODEL_WEIGHTS_PATH = os.path.join(PROJECT_ROOT, "outputs/coseg_v3_best.pth")
DEBUG_OUTPUT_DIR = os.path.join(PROJECT_ROOT, "debug_v3_output")
CONTEXT_SLICES = 1

# 後處理參數
BINARY_THRESHOLD = 0.5
Z_SMOOTH_SIGMA = 1.5
MIN_COMPONENT_VOXELS = 30
MORPH_CLOSE_RADIUS = 3
SAVE_DEBUG_IMAGES = True


def postprocess_3d(prob_volume):
    """Z 軸平滑 + 二值化 + 形態學閉合 + 連通區域過濾"""
    from scipy.ndimage import gaussian_filter1d, label as scipy_label

    # Z 軸平滑
    if Z_SMOOTH_SIGMA > 0 and prob_volume.shape[0] > 3:
        prob_volume = gaussian_filter1d(prob_volume, sigma=Z_SMOOTH_SIGMA, axis=0)

    # 二值化
    binary = (prob_volume > BINARY_THRESHOLD).astype(np.uint8)

    # 形態學閉合
    if MORPH_CLOSE_RADIUS > 0:
        kernel = cv2.getStructuringElement(
            cv2.MORPH_ELLIPSE, (MORPH_CLOSE_RADIUS * 2 + 1, MORPH_CLOSE_RADIUS * 2 + 1)
        )
        for z in range(binary.shape[0]):
            binary[z] = cv2.morphologyEx(binary[z], cv2.MORPH_CLOSE, kernel)

    # 連通區域過濾
    if MIN_COMPONENT_VOXELS > 0:
        labeled, n = scipy_label(binary)
        if n > 0:
            sizes = np.bincount(labeled.ravel())
            for comp_id in range(1, n + 1):
                if sizes[comp_id] < MIN_COMPONENT_VOXELS:
                    binary[labeled == comp_id] = 0

    return binary.astype(np.float32)


def main():
    device = "cuda" if torch.cuda.is_available() else "cpu"
    os.makedirs(DEBUG_OUTPUT_DIR, exist_ok=True)

    # ── 模型 ──
    hydra.core.global_hydra.GlobalHydra.instance().clear()
    hydra.initialize_config_module('sam2_configs', version_base='1.2')
    model = CoSeg(build_sam2("sam2_hiera_l.yaml", None, mode="train"))

    if torch.cuda.device_count() > 1:
        model = torch.nn.DataParallel(model)
    model = model.to(device)

    print(f"📦 載入: {MODEL_WEIGHTS_PATH}")
    state_dict = torch.load(MODEL_WEIGHTS_PATH, map_location=device)
    new_state_dict = {(k[7:] if k.startswith("module.") else k): v for k, v in state_dict.items()}

    if isinstance(model, torch.nn.DataParallel):
        model.module.load_state_dict(new_state_dict, strict=True)
    else:
        model.load_state_dict(new_state_dict, strict=True)

    model.eval()

    pixel_mean = np.array([123.675, 116.280, 103.530], dtype=np.float32)
    pixel_std = np.array([58.395, 57.12, 57.375], dtype=np.float32)

    patients = sorted([p for p in os.listdir(EVAL_DATA_DIR) if p.startswith("Patient_")])

    all_results = []

    print("=" * 80)
    print("🧠 CoSeg 3D 評估 — 含管狀結構指標")
    print("=" * 80)

    with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
        for patient_id in patients:
            img_dir = os.path.join(EVAL_DATA_DIR, patient_id, "image_1024")
            mask_dir = os.path.join(EVAL_DATA_DIR, patient_id, "mask_sem_1024")

            if not os.path.isdir(img_dir):
                continue

            img_files = sorted([f for f in os.listdir(img_dir) if f.endswith(".png")])
            if not img_files:
                continue

            # 預載灰度切片
            all_gray = {}
            for img_f in img_files:
                gray = cv2.imread(
                    os.path.join(img_dir, img_f), cv2.IMREAD_GRAYSCALE
                ).astype(np.float32)
                slice_idx = int(img_f.replace(".png", "").rsplit("_", 1)[-1])
                all_gray[slice_idx] = gray

            sorted_indices = sorted(all_gray.keys())

            prob_slices = {}
            gt_slices = {}

            for img_f in tqdm(img_files, desc=f"推論 {patient_id}"):
                slice_idx = int(img_f.replace(".png", "").rsplit("_", 1)[-1])
                center = all_gray[slice_idx]

                prev_slice = all_gray.get(slice_idx - CONTEXT_SLICES, center)
                next_slice = all_gray.get(slice_idx + CONTEXT_SLICES, center)
                img_3ch = np.stack([prev_slice, center, next_slice], axis=-1)

                gt_path = os.path.join(mask_dir, img_f.replace(".png", ".npy"))
                gt_mask = np.load(gt_path).astype(np.float32)

                img_norm = (img_3ch - pixel_mean) / (pixel_std + 1e-8)
                img_tensor = torch.tensor(img_norm).permute(2, 0, 1).unsqueeze(0).float().to(device)

                _, mask_pred_sem, _, _ = model(x=img_tensor)
                mask_pred_sem = F.interpolate(
                    mask_pred_sem, size=(1024, 1024),
                    mode='bilinear', align_corners=False
                )

                pred_prob = torch.sigmoid(mask_pred_sem[:, 0, :, :])[0].cpu().numpy()
                prob_slices[slice_idx] = pred_prob
                gt_slices[slice_idx] = (gt_mask > 0).astype(np.float32)

            # 組裝 3D
            prob_volume = np.stack([prob_slices[i] for i in sorted_indices], axis=0)
            gt_volume = np.stack([gt_slices[i] for i in sorted_indices], axis=0)

            # Raw
            raw_pred = (prob_volume > BINARY_THRESHOLD).astype(np.float32)

            # Post-processed
            post_pred = postprocess_3d(prob_volume.copy())

            # ── 計算所有指標 ──
            print(f"\n{'='*60}")
            print(f"📊 {patient_id}")
            print(f"{'='*60}")

            print(f"\n  Raw (無後處理):")
            raw_metrics = compute_all_metrics(raw_pred, gt_volume)
            for k, v in raw_metrics.items():
                if isinstance(v, float):
                    print(f"    {k:25s}: {v:.4f}")
                else:
                    print(f"    {k:25s}: {v}")

            print(f"\n  Post (含後處理):")
            post_metrics = compute_all_metrics(post_pred, gt_volume)
            for k, v in post_metrics.items():
                if isinstance(v, float):
                    print(f"    {k:25s}: {v:.4f}")
                else:
                    print(f"    {k:25s}: {v}")

            all_results.append({
                "patient": patient_id,
                **{f"raw_{k}": v for k, v in raw_metrics.items()},
                **{f"post_{k}": v for k, v in post_metrics.items()},
            })

            # ── Debug 圖 ──
            if SAVE_DEBUG_IMAGES:
                patient_debug_dir = os.path.join(DEBUG_OUTPUT_DIR, patient_id)
                os.makedirs(patient_debug_dir, exist_ok=True)

                for z_order, slice_idx in enumerate(sorted_indices):
                    gt_2d = gt_slices[slice_idx]
                    if gt_2d.sum() == 0:
                        continue

                    center_gray = all_gray[slice_idx].astype(np.uint8)
                    img_vis = np.stack([center_gray] * 3, axis=-1)

                    gt_vis = (gt_2d * 255).astype(np.uint8)
                    gt_vis_3c = np.stack([gt_vis] * 3, axis=-1)

                    pred_2d = post_pred[z_order]
                    pred_vis = (pred_2d * 255).astype(np.uint8)
                    pred_vis_3c = np.stack([pred_vis] * 3, axis=-1)

                    overlay = img_vis.copy()
                    overlay[gt_2d > 0, 1] = np.clip(
                        overlay[gt_2d > 0, 1].astype(np.int16) + 100, 0, 255
                    ).astype(np.uint8)
                    overlay[pred_2d > 0, 2] = np.clip(
                        overlay[pred_2d > 0, 2].astype(np.int16) + 100, 0, 255
                    ).astype(np.uint8)

                    combined = np.hstack((img_vis, gt_vis_3c, pred_vis_3c, overlay))
                    font = cv2.FONT_HERSHEY_SIMPLEX
                    cv2.putText(combined, 'Original', (20, 50), font, 1.5, (0, 255, 0), 3)
                    cv2.putText(combined, 'GT', (1024 + 20, 50), font, 1.5, (0, 255, 0), 3)
                    cv2.putText(combined, 'Pred', (2048 + 20, 50), font, 1.5, (0, 255, 0), 3)
                    cv2.putText(combined, 'Overlay', (3072 + 20, 50), font, 1.5, (0, 255, 0), 3)

                    cv2.imwrite(
                        os.path.join(patient_debug_dir, f"slice_{slice_idx:04d}.png"),
                        combined
                    )

    # ── 彙總 ──
    print("\n" + "=" * 80)
    print("📊 CoSeg 總結")
    print("=" * 80)

    metric_keys = ["dice", "iou", "hd95", "cldice", "connectivity_rate", "breakpoints", "centerline_distance"]
    metric_names = {
        "dice": "3D Dice",
        "iou": "3D IoU",
        "hd95": "HD95",
        "cldice": "clDice ⭐",
        "connectivity_rate": "Connectivity Rate",
        "breakpoints": "Breakpoints",
        "centerline_distance": "Centerline Distance",
    }

    for prefix, label in [("raw", "Raw (無後處理)"), ("post", "Post (含後處理)")]:
        print(f"\n  {label}:")
        print(f"  {'Metric':<30s} {'Mean':>10s} {'Std':>10s}")
        print(f"  {'-'*50}")
        for mk in metric_keys:
            key = f"{prefix}_{mk}"
            vals = [r[key] for r in all_results if key in r and r[key] != float('inf')]
            if vals:
                name = metric_names.get(mk, mk)
                if mk == "breakpoints":
                    print(f"  {name:<30s} {np.mean(vals):>10.1f} {np.std(vals):>10.1f}")
                else:
                    print(f"  {name:<30s} {np.mean(vals):>10.4f} {np.std(vals):>10.4f}")

    print("=" * 80)


if __name__ == "__main__":
    main()
