...
import hydra
from monai.losses import DiceFocalLoss  # 🌟 核心升級：換成更嚴厲的 Focal Loss
...
# 🌟 更改輸出檔名，保護舊的權重檔
OUTPUT_WEIGHTS = "outputs/coseg_public_mandibular_canal_focal_best.pth"
...
def main():
    ...
    # 🌟 已修正：移除不支援的 focal_weight 參數
    criterion = DiceFocalLoss(include_background=True, to_onehot_y=False, sigmoid=True)

    num_epochs = 80
    best_dice = 0.0

    print(f"🚀 開始訓練下顎管分割模型 (共 {num_epochs} Epochs，已啟用 Focal Loss 與陷阱題防禦)...")
